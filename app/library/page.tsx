...
export default function LibraryPage() {
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: `Listen to ${song.title} by ${song.artist} on VibeRush.`,
          url: songUrl,
        });
      } else {
  ...
  const createPlaylist = async () => {
    const token = await getToken();
    if (!token) return alert("Please log in to create playlists.");
    ...
    const data = await res.json();
    if (!res.ok) return alert(data?.error || "Could not create playlist.");
  ...
  const addToPlaylist = async (songId: string) => {
    const token = await getToken();
    if (!token) return alert("Please log in to add songs to playlists.");
    if (!selectedPlaylistId) return alert("Create or select a playlist first.");
  ...
  const fetchLikes = async () => {
    const token = await getToken();
    if (!token) return setLikedSongIds([]);
  ...
  const toggleLike = async (songId: string) => {
    const token = await getToken();
    if (!token) return alert("Please log in to like songs.");
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) return alert(data?.error || "Like failed.");
  ...
  const deleteSong = async (id: string) => {
    ...
    const token = await getToken();
    if (!token) return alert("Please log in before deleting.");
